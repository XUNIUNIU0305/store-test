$(function() {
    ...
    function login() {
        ...
        requestUrl('/index/login', 'POST', info, loginCB, errCB);
    }
    $('#J_login_btn').on('click.login', login)
    
    function toggleErrorMsg(msg, show, cb) {
        var $msg = $('.J_error_msg')
        if (!!show) $msg.html('<i class="glyphicon glyphicon-remove"></i> ' + msg).removeClass('invisible');
        else $msg.addClass('invisible');
        if (cb !== undefined) {cb()};
    }


    $('input').on('focus', function() {
        if (!$('.J_error_msg').hasClass('invisible')) {
            $('.J_error_msg').addClass('invisible')
        }
    })
