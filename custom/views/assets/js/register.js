...
$(function() {
	...
	$('#shop_name').focus();
	$('input').not('#shop_name').attr('disabled','disabled');
	$('.J_operator select').attr('disabled','disabled');
	$('.address select').attr('disabled','disabled');
	//判断注册码是否可用
	function judgeCode(val) {
		requestUrl('/register/checkaccountstatus', 'POST', {account: val}, function(data) {
			$('#shop_name').parents('.form-group').removeClass('error');
			$('input').attr('disabled',false);
			$('select').attr('disabled',false);
			$('.selectpicker').selectpicker('refresh');
    		$('.selectpicker').selectpicker('show');
		}, function(data) {
			$('#shop_name').parents('.form-group').addClass('error');
			$('input').not('#shop_name').attr('disabled','disabled');
			$('.J_operator select').attr('disabled','disabled');
			$('.address select').attr('disabled','disabled');
		})
    ...
    function ProvinceFn(data) {
    	...
    	$('select[class*=J_province]').off('.pro').on('change.pro', function() {
    		...
    		getMsg($('.login-wrap').data('src') + '/district/city', _data, CityFn)
    	})
    }
    //市级下拉函数
    function CityFn(data) {
    	...
    	if (data == '') {
    		...
    		getMsg($('.login-wrap').data('src') + '/district/district', _data, DistrictFn)
    		return
    	}
    	//市区联动
    ...
    //判断两次密码是否一致
    $('#shop_pwd2').on('blur', function() {
    	var passwd1 = $('#shop_pwd1').val();
    	var passwd2 = $('#shop_pwd2').val();
    	if (passwd1 != passwd2) {
    		$('#shop_pwd2').parents('.form-group').addClass('error');
    	}
    }).on('focus', function() {
    	$('#shop_pwd2').parents('.form-group').removeClass('error');
    })
    $('#shop_pwd1').on('blur', function() {
    	var passwd1 = $('#shop_pwd1').val();
    	var resultPW = passwd1.search(/[^0-9a-zA-Z]/g);
    	if (resultPW != -1) {
    		$('#shop_pwd1').parents('.form-group').addClass('error');
    	}
    }).on('focus', function() {
    	$('#shop_pwd1').parents('.form-group').removeClass('error');
    })
    //mobile
    ...
    $('.J_operator .selectpicker').on('change', function() {
    	$('.J_operator').removeClass('error')
    })
    $('.address .selectpicker').on('change', function() {
    ...
    $('.J_register_submit').on('click.submit', function() {
    	...
    	if (account == '' || passwd1 == '' || passwd2 == '' || mobile == '' || email == '') {
    		alert('请把信息填写完整！')
    		return
    	}
    	var resultPW = passwd1.search(/[^0-9a-zA-Z]/g);
    	...
    	requestUrl('/register/register', 'POST', _data, submitCB, errCB)
    })
