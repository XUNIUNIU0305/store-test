...
$(function() {
    ...
    function init() {
        ...
        function initCB(data) {
            _api_host = data.hostname;
            //获取省份信息
            var province_url = _api_host + '/district/province';
            //加载区域
            getMsg(province_url, '', ProvinceFn);
        }
    ...
    function ProvinceFn(data) {
        ...
        $('select[class*=J_province]').off('.pro').on('change.pro', function () {
            ...
            getMsg(_api_host + '/district/city', _data, CityFn)
        })

        if ( _profile.province && _profile.province.id != undefined) {
            $('select[class*=J_province]').val(_profile.province.id).trigger("change.pro");
        }
    }
    ...
    function CityFn(data) {
        ...
        if (data == '') {
            ...
            getMsg(_api_host + '/district/district', _data, DistrictFn)
            return
        }
        //市区联动
        ...
        if ( _profile.city && _profile.city.id != undefined) {
            $('select[class*=J_city]').val( _profile.city.id).trigger("change.cit");
        }
    }
    ...
    function DistrictFn(data) {
        ...
        if ( _profile.district && _profile.district.id != undefined) {
            $('select[class*=J_district]').val(_profile.district.id);
            refresh();
        }
    }
    ...
    $('input[type="file"]').on('change', function() {
        ...
        function succesCB(data) {
            if (data.status == 200) {
                $this.siblings('img').attr({'src': data.data.url});
                $this.siblings('img').data('filename', data.data.filename);
            } else {
    ...
    //判断当前账号审核状态
    function getInfo() {
        requestUrl('/account/auth/info', 'GET', '', function(data) {
            //判断是否有过信息
            if (data.status) {
                $('#shop_name').val(data.store_name);
                $('#company_name').val(data.corp_name);
                $('#email').val(data.email);
                $('#address').val(data.address);
                $('#responsor').val(data.manager_name);
                $('#contact').val(data.contact_name);
                $('#phone').val(data.contact_mobile);
                $('#J_business_licence').data('filename', data.business_licence.name);
                $('#J_business_licence').attr('src', data.business_licence.path);
                data.card_front.name && $('#J_card_front').data('filename', data.card_front.name);
                data.card_front.path && $('#J_card_front').attr('src', data.card_front.path);
                data.card_back.name && $('#J_card_back').data('filename', data.card_back.name);
                data.card_back.path && $('#J_card_back').attr('src', data.card_back.path);
                data.store_front.name && $('#J_store_front').data('filename', data.store_front.name);
                data.store_front.path && $('#J_store_front').attr('src', data.store_front.path);
                data.store_inside.name && $('#J_store_inside').data('filename', data.store_inside.name);
                data.store_inside.path && $('#J_store_inside').attr('src', data.store_inside.path);
                _profile = data.district
                init()
                $('#J_agreement').attr('checked', true);
                //审核中提示
                if (data.status == 2) {
                    $('#J_head_title').text('您的信息正在审核，请耐心等待')
                }
                if (data.status == 4) {
                    $('#J_head_title').text('恭喜，您的资质审核已通过！扫货特权将于审核通过后生效')
                }
                if (data.status == 5) {
                    $('#J_head_title').text('恭喜，您的资质审核已通过！扫货特权已经开通')
                }
                //非审核中禁止操作
                if (data.status != 3) {
                    $('#J_submit_info').addClass('disabled');
                    $('input[type="file"]').attr('disabled', 'disabled');
                }
                //审核拒绝留言
                if (data.status == 3) {
                    $('#J_head_title').text(data.comment)
                }
            }
        })
    }
    ...
    $('#J_submit_info').on('click', function() {
        if (!$('#J_agreement').is(':checked')) {
            alert('请确保您已阅读并同意《九大爷协议》！')
            return
        }
        var info = {};
        info.store_name = $('#shop_name').val().trim();
        info.corp_name = $('#company_name').val().trim();
        info.email = $('#email').val().trim();
        info.address = $('#address').val().trim();
        info.manager_name = $('#responsor').val().trim();
        info.contact_name = $('#contact').val().trim();
        info.contact_mobile = $('#phone').val().trim();
        info.business_licence = $('#J_business_licence').data('filename');
        info.card_front = $('#J_card_front').data('filename');
        info.card_back = $('#J_card_back').data('filename');
        info.store_front = $('#J_store_front').data('filename');
        info.store_inside = $('#J_store_inside').data('filename');
        info.province_id = $('#J_province').val()
        info.city_id = $('#J_city').val()
        info.district_id = $('#J_district').val()
        if (info.store_name == '') {
            alert('门店名称不能为空！')
            return
        }
        ...
        // var resultMB = info.contact_mobile.search(/0?(1)[0-9]{10}/);
        // if (resultMB == -1) {
        //     alert('手机号格式错误！')
        //     return
        // }
        var resultEM = info.email.search(/^[a-zA-Z0-9!#$%&\'*+\\/=?^_`{|}~-]+(?:\.[a-zA-Z0-9!#$%&\'*+\\/=?^_`{|}~-]+)*@(?:[a-zA-Z0-9](?:[a-zA-Z0-9-]*[a-zA-Z0-9])?\.)+[a-zA-Z0-9](?:[a-zA-Z0-9-]*[a-zA-Z0-9])?$/);
        if (resultEM == -1) {
            alert('邮箱格式不正确！')
            return
        }
        ...
        //     return
        // }
        // if (info.card_front == undefined || info.card_back == undefined) {
        //     alert('身份证图片未上传！')
        //     return
        // }
        // if (info.business_licence == undefined) {
        //     alert('营业执照未上传！')
        //     return
        // }
        // if (info.store_front == undefined || info.store_inside == undefined) {
        //     alert('门店照片不完整！')
        //     return
        // }
        requestUrl('/account/auth/submit', 'POST', info, function(data) {
            alert('上传成功！')
    ...
    //添加协议文本
    $('body').append(`
        <div class="apx-modal-agreement modal fade" id="apxModalAgreement" tabindex="-1">
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    <div class="modal-header">
                        <h4 class="modal-title">九大爷平台金牌会员服务协议</h4>
                    </div>
                    <div class="modal-body">
                        <p class="title">
                            本协议是您与九大爷平台（简称“平台”，网址：www.9daye.com.cn）所有者创智汇（苏州）电子商务有限公司（以下简称为“创智汇电子商务”）之间就九大爷平台会员服务等相关事宜所订立的契约，请您仔细阅读本注册协议，您点击“同意以下协议，提交”按钮后，本协议即构成对双方有约束力的法律文件。
                            本站的各项电子服务的所有权和运作权归平台所有。用户同意所有注册协议条款并完成注册程序，才能成为平台的会员。用户确认：本协议条款是处理双方权利义务的契约，始终有效，法律另有强制性规定或双方另有特别约定的，依其规定。
                        ...
                        <p class="detail">
                            2、有效期：会员有效期为1年，自审核通过之日起365天后自行失效，续费后可继续使用会员权益。
                        </p>
                        <h2>二、费用及启用条件</h2>
                        <p class="detail">
                            1、注册时支付的1元用于审核流程，如资质不符合条件将予以返还；
                        </p>
                        <p class="detail">
                            2、完成在线注册并通过相关资料审核。注册使用的资料须真实有效，包括：企业法人身份证、企业营业执照或具有门头为背景的门店名片。
                        </p>
                        <h2>三、会员权益</h2>
                        <p class="detail">
                            1、注册成功即赠送300元九大爷平台现金券一张及1.3升N+洗车液一桶（可洗1200台车），实物由工作人员送至门店交付；
                        </p>
                        <p class="detail">
                            2、会员推荐收益权：会员通过“推荐二维码”推荐其他单位完成注册，可获1.3升N+洗车液一桶，推荐次数不设上限。经推荐加入的新会员享有九大爷平台会员相同权益。
                        </p>
                        <p class="detail">
                            3、九大爷平台产品采购权，厂家批发价直邮到店。
                        </p>
                        <p class="detail">
                            4、年均100种优势产品信息推送。
                        </p>
                        <p class="detail">
                            5、提供多种线下线上免费培训，如“帮销季”，帮助门店实现产品销售和管理提升。
                        </p>
                        <h2>四、平台权限</h2>
                        <p class="detail">
                            平台在未登录状态下仅显示商品市场指导价，会员登陆平台后，可见实际采购价格。
                        </p>
                        <h2>五、会员资格的取消</h2>
                        <p class="detail">
                            如发现任何会员有以下故意行为之一，九大爷平台保留取消其使用服务的权利，并无需做出任何补偿；
                        </p>
                        <p class="detail">
                        ...
                        <p class="detail">
                            3、在本平台内从事非法商业行为，发布涉及敏感政治、宗教、色情或其它违反有关国家法律和政府法规的文字、图片等信息；
                        </p>
                        <p class="detail">
                            4、以任何非法目的而使用网络服务系统；
                        </p>
                        <h2>六、平台的权利</h2>
                        <p class="detail">
                            1、有权撤销或停止会员的全部或部分服务内容；
                        </p>
                        <p class="detail">
                            2、在有效期内保证会员既定权益的前提下，有权修订会员的其他权利和义务，有权修改或调整本平台的服务内容；
                        </p>
                        <p class="detail">
                            3、有权将修订的会员的权利和义务以e-mail或短信形式通知会员，会员收到通知后仍继续使用本平台服务者即表示会员同意并遵守新修订内容；
                        </p>
                        <p class="detail">
                            4、本平台提供的服务仅供会员独立使用，未经平台授权，会员不得将会员号授予或转移给第三方。会员如果有违此例，本平台有权向客户追索商业损失并保留追究法律责任的权利；
                        </p>
                        <h2>七、平台的义务</h2>
                        <p class="detail">
                        ...
                        <p class="detail">
                            2、 除不可抗拒的因素导致本平台临时停止或短时间停止服务以外，乙方如需停止本平台的全部或部分服务时，须提前在本平台上发布通知通告会员；
                        </p>
                        <p class="detail">
                        ...
                        <p class="detail">
                            5、平台不承担会员因遗失密码而受到的一切损失；
                        </p>
                        <h2>八、附则</h2>
                        <p class="detail">
                            1、以上规定的范围仅限于九大爷平台http://www.9daye.com.cn；
                        </p>
                        <p class="detail">
                            2、本网会员因违反以上规定而触犯有关法律法规，一切后果自负，本平台不承担任何责任；
                        </p>
                        <p class="detail">
                            3、本规则未涉及之问题参见有关法律法规，当本规定与有关法律法规冲突时，以相应的法律法规为准。在本条款规定范围内，九大爷平台拥有最终解释权；
                        </p>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-lg btn-default btn-agreement" data-dismiss="modal">关闭</button>
                    </div>
                </div>
            </div>
        </div>
