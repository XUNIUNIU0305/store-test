...
$(function () {
    ...
    function loadProfile() {
        function submitCB(data) {
            _profile = data;
            $("#shop_name").val(_profile.shopName);
            $("#nick_name").val(_profile.nickName);
            $("#email").val(_profile.email);
            $("#invoice_name").val(_profile.invoice_title);
            $("#invoice_no").val(_profile.invoice_number);
            if (_profile.headerImg != "") {
                $(".J_header_img img").attr("src", _profile.headerImg).removeClass("hidden");
              //  $(".J_header_img .close").removeClass("hidden");
             } else {
                $(".J_header_img").find("img").addClass("hidden").attr("src","");
               // $(".J_header_img .close").addClass("hidden");
            }
            $(".J_header_img img").removeClass('hide');
            init();
        }
        ...
        requestUrl('/account/profile/get-profile', 'GET', '', submitCB, errCB)
    }


    //绑定删除图片事件
    $(".J_header_img .close").off("click").on("click", function () {
        $(".J_header_img img").addClass("hidden").attr("src", "");
    ...
    /*保存基本信息*/
    $(".J_btn_save_profile").off("click.save").on("click.save", function () {
        ...
        var file_name = $(".J_header_img img").attr("data-filename");
        // 发票信息
        var invoice_name = $("#invoice_name").val();
        var invoice_no = $("#invoice_no").val();

        if (shop_name == '' || nick_name == '' || email == '' || verify_code == '') {
            showMessage("请填写完整的信息！");
            return false;
        }
        if (province == -1 || city == -1 || district == -1) {
            showMessage("请选择门店所属区域！");
            return
        }
        if (verify_code == '') {
        ...
        function errCB(data) {
            $('.J_btn_save_profile').removeClass('disabled');
            showMessage(data.data.errMsg)
        }

        $(this).addClass('disabled');
        requestUrl('/account/profile/save', 'POST', _data, submitCB, errCB)
    ...
    //上传
    $('.J_header_img').on('change', 'input[type="file"]', function (e) {

        var imgName = $(this).val();
        var $this = $(this);
        var suffix = getSuffix(imgName).toLowerCase();
        if(imgName==""){return;}
        if(suffix!="jpg"&&suffix!="gif"&&suffix!="png"){
            showMessage("图片格式错误,请选择JPG,PNG,GIF图片上传");return;
        }
        //上传成功回调处理
        function succesCB(data) {
            if (data.status == 200) {
                $(".J_header_img").find("img").removeClass("hidden").attr({"src":data.data.url,'data-filename':data.data.filename,});
                $(".J_header_img .close").removeClass("hidden");
                // $this.find('img').attr({'src': data.data.url, 'data-filename': data.data.filename});
            } else {
                showMessage(data.data.errMsg);
            }
        }
        ...
        function imgCB(data) {
         ...
         requestUrl('/account/profile/upload', 'GET', {file_suffix: suffix}, imgCB)
    })
    ...
    $('.J_get_verify_sms').click(function (e) {
        if (_profile.mobile == 0 || _profile.mobile.length == 0) {
            showMessage("您未绑定手机号码，请先绑定！");
            return;
        }
        ...
        requestUrl('/account/mobile/sendsms', 'GET', {type:4}, submitCB, errCB);
    })
    //加载用户数据
    loadProfile();
