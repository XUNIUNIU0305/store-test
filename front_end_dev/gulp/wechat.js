...
gulp.task('serve-wechat', ['watch-wechat'], function () {
    browserSyncInit([path.join(conf.paths.tmp, '/serve'), conf.paths.wechat]);
});

gulp.task('watch-wechat', ['wechat-inject', 'wechat-img'], function () {
    gulp.watch([
    ...
    gulp.watch([path.join(conf.paths.wechat, '/html/**/*.html'), path.join(conf.paths.wechat, '/html/*.inc')], function (event) {
        if (isOnlyChange(event)) {
...
gulp.task('wechat-htmlInclude', function (done) {
    return gulp.src(path.join(conf.paths.wechat, '/html/**/*.html'))
        .pipe(fileinclude({
            prefix: '@@',
            basepath: '@file'
        }))
...
gulp.task('wechat-styles', function () {
    ...
    return gulp.src([
            path.join(conf.paths.wechat, '/scss/app.scss')
        ])
...
gulp.task('wechat-inject', ['wechat-htmlInclude', 'wechat-scripts', 'wechat-styles'], function () {
