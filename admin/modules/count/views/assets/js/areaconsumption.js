...
$(function () {
    ...
    // 区域显示
    function getMapData(start, end, type) {
        requestUrl('http://115.29.196.61/api/get_data_one', 'GET', {str_time: start, end_time: end}, function(data) {
            //地图展示
            var regions_count_buy = [];
            var regions_count_cancel = [];
            var regions_sum_buy = [];
            var regions_sum_cancel = [];
            var vals_count_buy = [];
            var vals_count_cancel = [];
            var vals_sum_buy = [];
            var vals_sum_cancel = [];
            for (var i = 0; i < data.arr_count_buy.length; i++) {
                regions_count_buy.push(data.arr_count_buy[i].area);
                vals_count_buy.push(data.arr_count_buy[i].value);
            }
            for (var i = 0; i < data.arr_count_cancel.length; i++) {
                regions_count_cancel.push(data.arr_count_cancel[i].area);
                vals_count_cancel.push(data.arr_count_cancel[i].value);
            }
            for (var i = 0; i < data.arr_sum_buy.length; i++) {
                regions_sum_buy.push(data.arr_sum_buy[i].area);
                vals_sum_buy.push(data.arr_sum_buy[i].value)
            }
            for (var i = 0; i < data.arr_sum_cancel.length; i++) {
                regions_sum_cancel.push(data.arr_sum_cancel[i].area);
                vals_sum_cancel.push(data.arr_sum_cancel[i].value);
            }
            Array.prototype.max = function(){ 
                return Math.max.apply({},this) 
            } 
            var cbmax = vals_count_buy.max();
            var ccmax = vals_count_cancel.max();
            var sbmax = vals_sum_buy.max();
            var scmax = vals_sum_cancel.max();
            var mapChart = echarts.init(document.getElementById('mapChart'));
            
            var regionData_count_buy = [],regionData_count_cancel = [],regionData_sum_buy = [],regionData_sum_cancel = []; 
            regions_count_buy.forEach(function (region, i) {
                regionData_count_buy.push({
                    name: region,
                    value: vals_count_buy[i]
                })
            })
            regions_count_cancel.forEach(function (region, i) {
                regionData_count_cancel.push({
                    name: region,
                    value: vals_count_cancel[i]
                })
            })
            regions_sum_buy.forEach(function (region, i) {
                regionData_sum_buy.push({
                    name: region,
                    value: vals_sum_buy[i]
                })
            })
            regions_sum_cancel.forEach(function (region, i) {
                regionData_sum_cancel.push({
                    name: region,
                    value: vals_sum_cancel[i]
                })
            })
            var optionMap = {
                tooltip: {
                ...
                series: [
                    ...
                    },
                    {
                        ...
                        data: regionData_sum_cancel
                    }
                ]
            }
            mapChart.setOption(optionMap);
            mapChart.on('click', function (params) {
                    var type = params.seriesIndex + 1;
                    var area = params.name;
                    var data = {
                        str_time: $('.J_search_timeStart').val(),
                        end_time: $('.J_search_timeEnd').val(),
                        area: area,
                        type: type
                    }
                    // 图表显示
                    requestUrl('http://115.29.196.61/api/get_data_one_area', 'GET', data, function(data) {
                        var adate = [0];
                        var adata = [0];
                        for (var i = 0; i < data.length; i++) {
                            adate.push(data[i].date)
                            adata.push(data[i].value)
                        }
            ...
            mapChart.on('legendselectchanged', function (params) {
                ...
                if (params.name == '取消订单数') {
                    var newmax = ccmax;
                }
                if (params.name == '下订单金额') {
                    var newmax = sbmax;
                }
                if (params.name == '取消订单金额') {
                    var newmax = scmax;
                }
                var optionMap = {
                    tooltip: {
            ...
            //柱状图显示
            var bigBarChart = echarts.init(document.getElementById('bigBarChart'));
            regions_count_buy.push(0);
            regions_count_buy.unshift(0);
            vals_count_buy.push(0);
            vals_sum_buy.push(0);
            vals_count_buy.unshift(0);
            vals_sum_buy.unshift(0);
            var optionBigBar = {
                legend: {
                    orient: 'vertical',
                    left: 'left',
            ...
            bigBarChart.setOption(optionBigBar)
        })
    }
    
    getMapData(nowTime, nowTime)
    

    //查询
    $('#J_search_btn').on('click', function() {
       getMapData($('.J_search_timeStart').val(), $('.J_search_timeEnd').val());
       var barChart = echarts.init(document.getElementById('barChart'));
       var optionBar = {
            tooltip: {
                trigger: 'axis',
