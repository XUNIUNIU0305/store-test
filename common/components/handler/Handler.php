...
class Handler extends Component{
    ...
    public static function IpStrToLong(string $str){
        $long = 0;
        foreach(explode('.', $str) as $k => $int){
            $long += $int << (8 * $k);
        }
        return $long;
    }

    public static function IpLongToStr(int $long){
        $array = [];
        $array[] = $long >> 24;
        $array[] = ($long & 0x00FFFFFF) >> 16;
        $array[] = ($long & 0x0000FFFF) >> 8;
        $array[] = ($long & 0x000000FF);
