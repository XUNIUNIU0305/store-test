        ...
        try{
            ...
            foreach($innerCartesian as $singleCartesian){
                ...
                if($this->validateSkuRule){
                    $originalPrice = $outerCartesian[$singleCartesian]['original_price'] ?? 0;
                    $price = $outerCartesian[$singleCartesian]['price'] ?? 0;
                    if($originalPrice > $price) throw new \Exception;
                    if (!ProductSKUAR::find()->where([
                        'id' => $outerCartesian[$singleCartesian]['product_sku_id'],
