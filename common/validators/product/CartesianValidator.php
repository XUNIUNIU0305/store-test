    ...
    protected function validateValue($outerCartesian){
        ...
        try{
            ...
            foreach($innerCartesian as $singleCartesian){
                ...
                if (!isset($price)) {
                    $price = $itemPrice->generatePrice((float)$outerCartesian[$singleCartesian]['cost_price']);
                }
                if (!empty($outerCartesian[$singleCartesian]['original_price']) &&
                    $outerCartesian[$singleCartesian]['original_price'] < $price) throw new \Exception;
                if (!empty($outerCartesian[$singleCartesian]['original_guidance_price']) &&
                    $outerCartesian[$singleCartesian]['original_guidance_price'] < $outerCartesian[$singleCartesian]['guidance_price']) throw new \Exception;
                unset($price);
            }
            return true;
        }catch(\Exception $e){
