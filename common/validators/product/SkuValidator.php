    ...
    protected function validateValue($sku){
        ...
        try{
            foreach($sku as $skuId => $skuData){
                ...
                */
                $price = $itemPrice->generatePrice((float)$skuData['cost_price']);
                if (!empty($skuData['original_price']) &&
                    $skuData['original_price'] < $price) throw new \Exception;
                if (!empty($skuData['original_guidance_price'] ) &&
                    $skuData['original_guidance_price'] < $skuData['guidance_price']) throw new \Exception;
                unset($price);
            }
